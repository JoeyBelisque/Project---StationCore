...
function normalizeHeadsetLifecycle(rawStatus, rawObservacao = "") {
  ...
  if (statusNorm === "disponivel") return { status: "estoque", categoria: "estoque", observacoes: observacao };
...
function normalizeColumnNames(row) {
  ...
  const normalizeKey = (key) =>
    key.toLowerCase().trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[º°]/g, 'o').replace(/[^a-z0-9\s]/g, '').replace(/\s+/g, '_').replace(/_+/g, '_');
  const normalizeLoose = (key) => key.toLowerCase().replace(/[^a-z]/g, '');

  for (const [key, value] of Object.entries(row)) columnMap[normalizeKey(key)] = value;

  const fieldMap = {
    matricula: ['matricula'],
    nome_operador: ['operador', 'nome_operador', 'usuario_nome', 'colaborador', 'funcionario'],
    nome: ['identificador', 'nome_headset', 'nome_equipamento', 'nome_dispositivo'],
    lacre: ['lacre'],
    marca: ['marca'],
    numero_serie: ['numero_serie', 'numero_de_serie', 'n_serie', 'nserie', 'n_de_serie', 'no_serie', 'num_serie', 'ns'],
    status: ['status'],
  ...
  for (const [fieldName, aliases] of Object.entries(fieldMap)) {
    for (const alias of aliases) { if (alias in columnMap) { result[fieldName] = columnMap[alias]; break; } }
  }

  const keys = Object.keys(columnMap);
  if (!result.numero_serie) {
    const found = keys.find(k => { const clean = normalizeLoose(k); return clean.includes("serie") && (clean.includes("n") || clean.includes("num") || clean.includes("numero")); });
    if (found) result.numero_serie = columnMap[found];
  }
  return result;
}
...
function collectComputadores(rows) {
  ...
  rows.forEach((raw, idx) => {
    ...
    if (hostname) {
      ...
      if (seenHostname.has(k)) errors.push(rowError("computadores", line, `hostname duplicado: ${hostname}`));
      seenHostname.add(k);
    }
    if (serial_number) {
      const k = serial_number.toLowerCase();
      if (seenSerial.has(k)) errors.push(rowError("computadores", line, `n serie duplicado: ${serial_number}`));
      seenSerial.add(k);
    }
...
async function validateComputadoresAgainstDatabase(computadores) {
  ...
  if (hostnames.length || serials.length) {
    const result = await pool.query(`SELECT hostname, serial_number FROM computadores WHERE hostname = ANY($1::text[]) OR serial_number = ANY($2::text[])`, [hostnames, serials]);
    const hostDb = new Set(result.rows.map(r => r.hostname.toLowerCase()));
    const serialDb = new Set(result.rows.map(r => r.serial_number?.toLowerCase()));
    computadores.forEach((row) => {
      if (row.hostname && hostDb.has(row.hostname.toLowerCase())) errors.push(rowError("computadores", row.line, `hostname ja existe: ${row.hostname}`));
      if (row.serial_number && serialDb.has(row.serial_number.toLowerCase())) errors.push(rowError("computadores", row.line, `n serie ja existe: ${row.serial_number}`));
    });
  }
  return errors;
}
  ...
  try {
    ...
    for (const row of headsets) {
      ...
      if (existing.rowCount === 0) {
        ...
        await client.query(`UPDATE headsets SET matricula = $2, nome_operador = $3, nome = $4, marca = $5, numero_serie = $6, status = $7, categoria = $8, observacoes = $9, data_devolucao = $10, updated_at = NOW() WHERE id = $1`, [atual.id, proxMat, proxNomOp, proxNomHs, row.marca || atual.marca, row.numero_serie || atual.numero_serie, proxStat, proxCat, row.observacoes || atual.observacoes, proxDev]);
      }
    }
...
async function persistComputadores(computadores) {
  ...
  try {
    await client.query("BEGIN");
    for (const row of computadores) { await client.query(`INSERT INTO computadores (hostname, serial_number, status, pa) VALUES ($1, $2, $3, $4)`, [row.hostname, row.serial_number, row.status, row.pa]); }
    await client.query("COMMIT");
  } catch (error) { await client.query("ROLLBACK"); throw error; } finally { client.release(); }
}
